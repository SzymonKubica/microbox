#include "lcd_24_display.hpp"
#include "../../../lib/lcd_2_4_inch/GUI_Paint.h"
#include "../../../lib/lcd_2_4_inch/LCD_Driver.h"

#define DISPLAY_CORNER_RADIUS 40
#define SCREEN_BORDER_WIDTH 3
void Lcd24Display::setup()
{
        Config_Init();
        LCD_Init();
        LCD_SetBackLight(50);
...
void Lcd24Display::initialize()
{
        Paint_NewImage(LCD_WIDTH, LCD_HEIGHT, 270, WHITE);
};

void Lcd24Display::clear(Color color) { Paint_Clear(color); };

void Lcd24Display::draw_rounded_border(Color color)
{
...
void Lcd24Display::draw_circle(Point center, int radius, Color color,
...
void Lcd24Display::draw_rectangle(Point start, int width, int height, Color color,
                                int border_width, bool filled)
{
...
void Lcd24Display::draw_rounded_rectangle(Point start, int width, int height,
...
void Lcd24Display::draw_line(Point start, Point end, Color color)
{
        /*
         * We need to add the adjustment because of the pixel-precision
         * inconsistency between the sfml emulator and the lcd display. The
...
void Lcd24Display::draw_string(Point start, char *string_buffer,
...
void Lcd24Display::clear_region(Point top_left, Point bottom_right,
                              Color clear_color)
...
int Lcd24Display::get_height() { return LCD_WIDTH; };

// We return the height for the width as the display is mounted horizontally.
int Lcd24Display::get_width() { return LCD_HEIGHT; };

int Lcd24Display::get_display_corner_radius() { return DISPLAY_CORNER_RADIUS; }

bool Lcd24Display::refresh()
{
